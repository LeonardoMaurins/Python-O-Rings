...
def threshold_value(image): 
    hist = image_hist(image[1]) # Retrieves histogram of pixel values for the given image
    peaks = hist_peaks(hist) # Retrieve the 2 peaks of pixel values in the histogram
...
for image in images: 
    ...
    results = start_process(image[0].split('/')[-1], image_threshold, image_labels, centroid, bounding_box, start, results)
